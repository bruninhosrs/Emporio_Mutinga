...
exports.withdrawFromCashRegister = async (req, res) => {
  try {
    ...
    cashRegister.totalWithdrawals =
      parseFloat(cashRegister.totalWithdrawals || 0) + withdrawalAmount;
    cashRegister.openingBalance -= withdrawalAmount;

    await cashRegister.save();
    res
      .status(200)
      .json({ message: "Sangria realizada com sucesso", cashRegister });
  } catch (error) {
    ...
    if (cashRegister) {
      ...
      if (cashRegister.status === "fechado") {
        ...
        return res
          .status(200)
          .json({
            message: "Caixa reaberto com sucesso e saldo inicial zerado.",
            cashRegister,
          });
      }
...
exports.closeCashRegister = async (req, res) => {
  try {
    ...
    const closingBalance =
      parseFloat(cashRegister.openingBalance) +
...
exports.getCashRegisterBalance = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ message: "Erro ao consultar saldo.", error: error.message });
  }
};
...
exports.getSalesByMethod = async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({
        message: "Erro ao consultar os métodos de pagamento.",
        error: error.message,
      });
  }
};
